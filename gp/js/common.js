...
function showPopupNotification(strNotification)
{
	...
	popupNotification.innerHTML += '<br><br><input type="button" class="normalButton" height="30px" width="60px" onclick="closePopupNotification();" value="확인">';
...
function showFullScreen()
{
	...
	var strNotification = "Full Screen Test를 위해 Full Screen을 기본 화면 위에 Overlay합니다.";
	strNotification += "<br>두 화면이 겹쳐서 나오는 것은 테스트를 위한 것이지 TV의 오동작이 아닙니다.";
...
addEventListener("load", initMouseMoveEvent, true);
	...
	function setSelectedButton(cnt) {
		...
		var menuList = document.querySelectorAll("input[type=button]");
		for(var i=0 in menuList) {
			if(parseInt(selectedButton.value)+cnt == parseInt(menuList[i].id)) {
				selectedButton.id = parseInt(selectedButton.value)+"_button";
				menuList[i].id = "selected";
			}
		}
	};
